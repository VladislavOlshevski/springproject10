...
public interface ProjectRepository extends CrudRepository<Project, Long> {
    ...
    Optional<Project> findByNameAndUser(@Param("name") String name,
    ...
    Optional<Project> findByLoginAndName(@Param("login") String login,
                                         @Param("name") String name);
    @Query("select p from Project p join p.user u join p.projectInfo i " +
            "where u = :user and i.name like :search")
    List<Project> findContaining(@Param("user") User user, @Param("search")
